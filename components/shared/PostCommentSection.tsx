...
export function PostCommentSection({ postId }: CommentSectionProps) {
  ...
  const fetchComments = async () => {
    setLoading(true);
    try {
      const response = await petPostService.getComments(postId);
      if (response.success) {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await petPostService.addComment(postId, commentContent);
      if (response.success) {
        setCommentContent("");
        toast({
          type: "success",
    ...
    <div className="space-y-8">
      ...
      <form
        ...
        <textarea
          ...
          className="w-full min-h-[100px] bg-muted/20 border-transparent rounded-2xl p-4 text-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all resize-none outline-none"
        />
